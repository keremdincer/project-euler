...
func collatzSequence(number int, length int) int {
	if number == 1 {
		return length
	}

	if number%2 == 0 {
		return collatzSequence(number/2, length+1)
	}

	return collatzSequence(number*3+1, length+1)
}

func main() {
	longestSequence := 0
	longestSequenceStarter := 0

	for i := 13; i < 1000000; i++ {
		result := collatzSequence(i, 1)
	...
	fmt.Printf("%d: %d", longestSequenceStarter, longestSequence)
}
